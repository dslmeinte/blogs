...
So, I wasn't too busy to drop what I was doing, especially not since "`the country needs saving`" -- not my words, but those of Ron Roozendaal, then CTO for the Dutch Ministry of Health, who coordinated the COVID-19 response there.

This was the sparked that jump-started my most interesting job sofar in my career.
...
I've already spoken about this experience at a couple of conferences - see e.g. link:LangDev-2022.pdf[this slidedeck for the LangDev 2022 conference].
But I felt it'd be good to write it down as something of a longer story that might serve as anecdotal documentation, might provide some insight on design decisions made -- insight which might come in handy during future pandemics.
...
<5> The `ci` field contains an https://lexparency.org/eu/32021D1073/ANX_III/[Unique Certificate Identifier] that uniquely identifies the precise vaccination.
...
The obvious urgency played a big role, but I think a bigger part of the success can be attributed to the involvement of, and cooperation between, a relatively small number of high-quality software engineers who were then given a great deal of autonomy, devoid of the usual bureaucratics.
...
Those are huge numbers for something that was developed from scratch early 2021!
====
...
Under EU's principle of sovereignty, each country are free to establish their own fit-for-entry regulations that determine when a person holding an EU DCC can enter that country.
...
In principle, each country could simply implement their entry regulation in their own scanner apps, using regular code.
(And many countries did just that, for varying reasons.)
But there's a couple of problems with that reasoning:
...
The primary example is hackers having produced fake DCCs from fake German pharmacies, but for the rest it's been rarely used.
...
On the other hand, several participating countries have never published business rules precisely because they couldn't fit their entire entry regulations in the framework.
They felt that publishing only a part of their entry regulations amounting to misleading travelers: an app evaluating the "`partial`" business rules could've deemed a traveler fit-for-entry, while the border agent would turn them back.


== Specifying logic with CertLogic

Not that I've set up the context, I can tell you what my role in all of this was.
...
Doing that doesn't violate the "`no bytecode`"-policy for apps which exists on some mobile platforms -- most notably, Apple's.
...
I also added a couple of operations to CertLogic that weren't in JsonLogic, specifically for working with dates and datetimes.
...
But these operation would've more often than not been used in direct combination with each other.
...
My "`dayjob`" of implementing Domain-Specific Languages (**DSL**s) is probably part of the reason why.
...
Keeping the CertLogic language as small, simple, and focused as possible was absolutely key to achieve correctness and security within the very short timeframe available.
...
It gets already murkier with the number of properties/features of concepts: should we look at their total number, their average number, the maximum number of properties among all concepts?
I'm not sure there's much use in trying to going scientific here: you'll probably recognise something as "`small`" when it obviously is -- and when it isn't recognisably small, it probably isn't.
====
...
We've seen quite a bit of variation in ways in which certain criteria have been implemented.
...
(Initially, these identifier were called Universal Vaccination Certificate Identifier, with the abbrevation UVCI still lingering.)
An UCI can be essentially opaque -- as is the case in the example above -- , but it can also be structured to e.g. identify the vaccination facility through one of its fragments.
...
Someone first suggested to "`just`" add a regex operation to CertLogic to detect such UCIs.
...
In particalur, I wanted to be able to derive for each country that had published business rules which vaccines they accepted, and what delay/wait times applied for those.

Initially, I ran the analysis just by running lots of input data at a set of business rules.
In particular, for a particular combination of vaccine, _<``dn``>_/_<``sd``>, I let the vaccination date run across a couple of years with increments of one day.
...
(Before that, I also implemented a https://github.com/ehn-dcc-development/eu-dcc-business-rules/tree/main/certlogic/certlogic-utils-js/src/compiler[compiler] which speeded things up nicely, but still had the second problem described above.)
...
To make the analysis work, I had to make https://github.com/ehn-dcc-development/eu-dcc-business-rules-analysis/blob/main/src/analyser/replacements.json["`replacements`"]: occurrences of certain sub expressions in the business rules set of a specific country would be replaced by simpler expression.
...
It's usually not even to difficult to implement.
...
.. Each DCC has a expiry date -- typically a full year for a vaccination- or recovery-DCC, but significantly less for a test-DCC.
...
Directly after scanning is verifies the DCC's authenticity, and decides whether the person providing the DCC is allowed to be where they are.
...
In practice, there's many reasons why this doesn't work out.
...
* A more subtle problem occurs for people that need more vaccinations than other people, such as with immunocompromised people.
...
It'd be good if we had been able to replace it with a proof of sufficient immunisation according to a defined protocol.
...
. __A proper IDE for authoring business rules with, including test facilities.__
+
...
I made a https://non-dcc-rules-prototype.vercel.app/[proposal implementation] of such an extended standard, but the interest and need was already waning at that point.
...
Partly it was because we were going for a more generic framework -- see previous item.
...
Expressions of logical conditions in a JSON format like CertLogic quickly becomes large, which means that authoring and amending quickly becomes tedious, error-prone, and complex.
...
A Web IDE -- especially one that could be run completely locally/off-line -- would've had a much better chance.
